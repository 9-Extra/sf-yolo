...
def adjust_learning_rate(init_lr, lr_decay, optimizer, iteration_count):
    """Imitating the original implementation"""
    lr = init_lr / (1.0 + lr_decay * iteration_count)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["voc2clipart", "voc2wc", "city2foggy", "KC", "city", "bdd100k"],
        help="choose one, voc2clipart, voc2wc, city2foggy, city, bdd100k and KC",
    )
    parser.add_argument(
        "--content_dir",
        type=str,
    ...
    parser.add_argument(
        "--save_dir", default="./models", help="Directory to save the model"
    )
    ...
    assert args.scenario_name in [
        ...
        "city",
        "bdd100k",
    ], "please choose one from voc2clipart, voc2wc, city2foggy, city, bdd100k and KC"
    ...
    for i in tqdm(range(args.max_iter)):
        adjust_learning_rate(
            args.lr_decoder, args.lr_decay, optimizer1, iteration_count=i
        )

        content_images = next(content_iter).to(device)
        style_images = next(style_iter).to(device)

        loss_c, loss_const = network(
            content_images, style_images, args.scenario_name, flag=0
        )
        ...
        if i >= args.before_fcs_steps:
            adjust_learning_rate(
                args.lr_fcs, args.lr_decay, optimizer2, iteration_count=i - args.before_fcs_steps
            )
            loss_s_1, loss_s_2 = network(
                content_images, style_images, args.scenario_name, flag=1
            )
        ...
        if save:
            ...
            torch.save(
                state_dict, args.save_dir + "/decoder_iter_{:d}.pth".format(i + 1)
            )
            state_dict = net.fc1.state_dict()
            for key in state_dict.keys():
                state_dict[key] = state_dict[key].to(torch.device("cpu"))
            torch.save(state_dict, args.save_dir + "/fc1_iter_{:d}.pth".format(i + 1))
            state_dict = net.fc2.state_dict()
            for key in state_dict.keys():
                state_dict[key] = state_dict[key].to(torch.device("cpu"))
            torch.save(state_dict, args.save_dir + "/fc2_iter_{:d}.pth".format(i + 1))
